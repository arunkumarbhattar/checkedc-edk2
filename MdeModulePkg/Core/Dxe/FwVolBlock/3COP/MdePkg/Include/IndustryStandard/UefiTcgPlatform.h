...
typedef struct tdTCG_PCR_EVENT {
  ...
  UINT8 Event _Checked[1];                   ///< The event data
} TCG_PCR_EVENT;
...
typedef struct tdEFI_IMAGE_LOAD_EVENT {
  ...
  EFI_DEVICE_PATH_PROTOCOL DevicePath _Checked[1];
} EFI_IMAGE_LOAD_EVENT;
...
typedef struct tdEFI_HANDOFF_TABLE_POINTERS {
  ...
  EFI_CONFIGURATION_TABLE TableEntry _Checked[1];
} EFI_HANDOFF_TABLE_POINTERS;
...
typedef struct tdEFI_VARIABLE_DATA {
  ...
  CHAR16 UnicodeName _Checked[1];
  INT8 VariableData _Checked[1];                        ///< Driver or platform-specific data
} EFI_VARIABLE_DATA;
...
typedef struct {
  ...
  CHAR16 UnicodeName _Checked[1];
  INT8 VariableData _Checked[1];
} EFI_VARIABLE_DATA_TREE;

typedef struct tdEFI_GPT_DATA {
  EFI_PARTITION_TABLE_HEADER    EfiPartitionHeader;
  UINTN                         NumberOfPartitions;
  EFI_PARTITION_ENTRY Partitions _Checked[1];
} EFI_GPT_DATA;
