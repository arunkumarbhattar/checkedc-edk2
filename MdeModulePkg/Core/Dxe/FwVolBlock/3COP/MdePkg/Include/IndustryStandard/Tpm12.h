...
typedef struct tdTPM_DIGEST {
  UINT8 digest _Checked[TPM_SHA1_160_HASH_LEN];
} TPM_DIGEST;
...
typedef struct tdTPM_KEY_HANDLE_LIST {
  UINT16            loaded;
  TPM_KEY_HANDLE handle _Checked[1];
} TPM_KEY_HANDLE_LIST;
...
typedef struct tdTPM_MSA_COMPOSITE {
  UINT32        MSAlist;
  TPM_DIGEST migAuthDigest _Checked[1];
} TPM_MSA_COMPOSITE;
...
typedef struct tdTPM_STCLEAR_DATA {
  ...
  TPM_PCRVALUE PCR _Checked[TPM_NUM_PCR];
...
typedef struct tdTPM_PCR_COMPOSITE {
  ...
  UINT32               valueSize;
  TPM_PCRVALUE pcrValue _Checked[1];
} TPM_PCR_COMPOSITE;
...
typedef struct tdTPM_MIGRATE_ASYMKEY {
  ...
  _Ptr<UINT8> partPrivKey;                 //  45 112-127 157-172
} TPM_MIGRATE_ASYMKEY;
...
typedef struct tdTPM_IDENTITY_PROOF {
  ...
  _Ptr<UINT8> labelArea;
  _Ptr<UINT8> identityBinding;
  _Ptr<UINT8> endorsementCredential;
  _Ptr<UINT8> platformCredential;
  _Ptr<UINT8> conformanceCredential;
} TPM_IDENTITY_PROOF;
...
typedef struct tdTPM_CONTEXT_BLOB {
  ...
  TPM_HANDLE           handle;
  UINT8 label _Checked[16];
  UINT32               contextCount;
  TPM_DIGEST           integrityDigest;
  UINT32               additionalSize;
  _Ptr<UINT8> additionalData;
  UINT32               sensitiveSize;
  _Ptr<UINT8> sensitiveData;
} TPM_CONTEXT_BLOB;
...
typedef struct tdTPM_FAMILY_TABLE {
  TPM_FAMILY_TABLE_ENTRY famTableRow _Checked[TPM_NUM_FAMILY_TABLE_ENTRY_MIN];
} TPM_FAMILY_TABLE;
...
typedef struct tdTPM_DELEGATE_TABLE {
  TPM_DELEGATE_TABLE_ROW delRow _Checked[TPM_NUM_DELEGATE_TABLE_ENTRY_MIN];
} TPM_DELEGATE_TABLE;
...
typedef struct tdTPM_DELEGATE_OWNER_BLOB {
  ...
  TPM_DIGEST             integrityDigest;
  UINT32                 additionalSize;
  _Ptr<UINT8> additionalArea;
  UINT32                 sensitiveSize;
  _Ptr<UINT8> sensitiveArea;
...
typedef struct tdTPM_CAP_VERSION_INFO {
  ...
  UINT16               specLevel;
  UINT8                errataRev;
  UINT8 tpmVendorID _Checked[4];
  UINT16               vendorSpecificSize;
  _Ptr<UINT8> vendorSpecific;
...
typedef struct tdTPM_DAA_ISSUER {
  ...
  UINT8 DAA_generic_q _Checked[26];
} TPM_DAA_ISSUER;
...
typedef struct tdTPM_DAA_CONTEXT {
  ...
  UINT8 DAA_scratch _Checked[256];
  UINT8                   DAA_stage;
} TPM_DAA_CONTEXT;
...
typedef struct tdTPM_DAA_JOINDATA {
  UINT8 DAA_join_u0 _Checked[128];
  UINT8 DAA_join_u1 _Checked[138];
