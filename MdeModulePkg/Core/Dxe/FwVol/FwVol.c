...
ReadFvbData (
        ...
        IN OUT EFI_LBA                             *StartLba : itype(_Ptr<EFI_LBA>),
        IN OUT UINTN                               *Offset : itype(_Ptr<UINTN>),
        IN     UINTN                               DataSize,
        OUT    UINT8                               *Data : itype(_Array_ptr<UINT8>) byte_count(DataSize)
)
{
UINTN       BlockSize;
  ...
  )
{
  ...
  EFI_LBA                     StartLba;
  UINTN                       Offset;
  _Array_ptr<UINT8>           Buffer : byte_count(FvhLength) = NULL;
  ...
  *FwVolHeader = AllocatePool (TempFvh.HeaderLength);
  ...
  Buffer    = _Assume_bounds_cast<_Array_ptr<UINT8>>(*FwVolHeader + sizeof (EFI_FIRMWARE_VOLUME_HEADER), count(FvhLength));
  ...
  )
{
  ...
  BOOLEAN                             FileCached;
  UINTN                               WholeFileSize;
  EFI_FFS_FILE_HEADER*           CacheFfsHeader = NULL;
  ...
  if (FwVolHeader->ExtHeaderOffset != 0) {
    ...
    FfsHeader      = FwVolExtHeader + FwVolExtHeader->ExtHeaderSize;
  } else {
    FfsHeader = FvDevice->CachedFv + FwVolHeader->HeaderLength;
  }

  FfsHeader    = ALIGN_POINTER ((VOID*)FfsHeader, 8);
  ...
  while (((UINTN)FfsHeader >= (UINTN)FvDevice->CachedFv) && ((UINTN)FfsHeader <= (UINTN)((UINTN)TopFvAddress - sizeof (EFI_FFS_FILE_HEADER)))) {
    ...
    if ((CacheFfsHeader->Attributes & FFS_ATTRIB_CHECKSUM) == FFS_ATTRIB_CHECKSUM) {
      if (FvDevice->IsMemoryMapped) {
        ...
        CacheFfsHeader = AllocateCopyPool (WholeFileSize, (void*)CacheFfsHeader);
